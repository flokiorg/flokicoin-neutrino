...
type MockChainClient struct {
	getBlockResponse     map[chainhash.Hash]*chainutil.Block
...
func NewMockChainClient() *MockChainClient {
	return &MockChainClient{
		getBlockResponse:     make(map[chainhash.Hash]*chainutil.Block),
		getBlockHashResponse: make(map[int64]*chainhash.Hash),
		getCFilterResponse:   make(map[chainhash.Hash]*gcs.Filter),
	}
}

func (c *MockChainClient) SetBlock(hash *chainhash.Hash, block *chainutil.Block) {
...
func TestUtxoScannerScanBasic(t *testing.T) {
	...
	mockChainClient.SetBlock(&block99999Hash, chainutil.NewBlock(&Block99999))

	block100000Hash := Block100000.BlockHash()
	mockChainClient.SetBlockHash(100000, &block100000Hash)
	mockChainClient.SetBlock(&block100000Hash, chainutil.NewBlock(&Block100000))
...
func TestUtxoScannerCancelRequest(t *testing.T) {
	...
	scanner := NewUtxoScanner(&UtxoScannerConfig{
		GetBlock: func(chainhash.Hash,
			...QueryOption) (*chainutil.Block, error) {
...
// The following is taken from the flokicoin/chainutil project.
