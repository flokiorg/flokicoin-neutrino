...
// Request is the main struct that defines a flokicoin network query to be sent to
// connected peers.
type Request struct {
...
// WorkManager defines an API for a manager that dispatches queries to flokicoin
...
type Peer interface {
	// QueueMessageWithEncoding adds the passed flokicoin message to the peer
	// send queue.
