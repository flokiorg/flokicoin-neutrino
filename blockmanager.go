...
type blockManagerCfg struct {
	...
	// QueryDispatcher is used to make queries to connected Flokicoin peers.
...
// newBlockManager returns a new flokicoin block manager.  Use Start to begin
...
func resolveFilterMismatchFromBlock(block *wire.MsgBlock,
	...
	for peerAddr, filter := range filtersFromPeers {
		...
		numOpReturns, err := VerifyBasicBlockFilter(
			filter, chainutil.NewBlock(block),
		)
		if err != nil {
