...
var (
	// Try flog.LevelInfo for output like you'd see in normal operation,
	// or flog.LevelTrace to help debug code. Anything but
	// flog.LevelOff turns on log messages from the tests themselves as
	// well. Keep in mind some log messages may not appear in order due to
	// use of multiple query goroutines in the tests.
	logLevel    = flog.LevelOff
...
func init() {
	...
	logger := flog.NewBackend(os.Stdout)
...
// secSource is an implementation of walletd/txauthor/SecretsSource that
...
func (s *secSource) add(privKey *crypto.PrivateKey) (chainutil.Address, error) {
...
func (s *secSource) GetKey(addr chainutil.Address) (*crypto.PrivateKey, bool,
	error) {
...
func testRescan(harness *neutrinoHarness, t *testing.T) {
	...
	newPkScript := func() (chainutil.Address, []byte, *wire.TxOut) {
		t.Helper()
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target chainutil.Amount) (
		...
		return func(target chainutil.Amount) (chainutil.Amount,
			[]*wire.TxIn, []chainutil.Amount,
			[][]byte, error) {
	...
	privKey3, err := crypto.NewPrivateKey()
	if err != nil {
		t.Fatalf("Couldn't generate private key: %s", err)
	}
	...
	authTx1, err := txauthor.NewUnsignedTransaction(
		...
		// Fee rate is lokis per kilobyte
		1024000,
		inSrc(*tx1),
...
func TestNeutrinoSyncWithHeadersImport(t *testing.T) {
	...
	err = chainimport.AddHeadersImportMetadata(
		blockHeadersImportPath, chaincfg.SimNetParams.Net, 0,
		headerfs.Block, 0,
	)
	require.NoError(t, err)

	err = chainimport.AddHeadersImportMetadata(
		filterHeadersImportPath, chaincfg.SimNetParams.Net, 0,
	...
	// Generate an additional 30 blocks on h1. This ensures that when we
	// sync again, the client will need to fetch these new blocks from the
	// network.
	_, err = h1.Client.Generate(30)
	require.NoError(t, err)
	t.Log("Syncing again after generating more 30 blocks on h1")
...
//
// NOTE: this requires testdata/block-headers-mainnet-1-100_000.bz2 and
// testdata/filter-headers-mainnet-1-100_000.bz2, real Flokicoin mainnet
// header dumps (100k headers) matching chaincfg.MainNetParams. These aren't
// checked into this fork: the upstream fixtures are Bitcoin mainnet data,
// which fails our chainimport's network-magic/genesis validation outright.
