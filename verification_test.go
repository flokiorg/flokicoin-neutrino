...
func TestVerifyBlockFilter(t *testing.T) {
	...
	b := chainutil.NewBlock(spendBlock)
...
func spendP2PK(_ *testing.T, _ *crypto.PublicKey, prevTx *wire.MsgTx,
...
func spendP2PKH(_ *testing.T, pubKey *crypto.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func spendP2SH(t *testing.T, pubKey *crypto.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func spendNP2WKH(t *testing.T, pubKey *crypto.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {

	pkHash := chainutil.Hash160(pubKey.SerializeCompressed())

	witAddr, err := chainutil.NewAddressWitnessPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func spendP2WKH(_ *testing.T, pubKey *crypto.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func spendP2WSH(t *testing.T, pubKey *crypto.PublicKey, prevTx *wire.MsgTx,
	idx uint32) *wire.TxIn {
...
func makeP2PK(t *testing.T, pubKey *crypto.PublicKey) []byte {
	addr, err := chainutil.NewAddressPubKey(
...
func makeP2PKH(t *testing.T, pubKey *crypto.PublicKey) []byte {
	pkHash := chainutil.Hash160(pubKey.SerializeCompressed())
	addr, err := chainutil.NewAddressPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2SH(t *testing.T, pubKey *crypto.PublicKey) []byte {
	script := scriptP2PKH(t, pubKey)

	addr, err := chainutil.NewAddressScriptHash(script, chainParams)
	require.NoError(t, err)
...
func makeNP2WKH(t *testing.T, pubKey *crypto.PublicKey) []byte {
	pkHash := chainutil.Hash160(pubKey.SerializeCompressed())
	...
	addr, err := chainutil.NewAddressScriptHash(witnessProgram, chainParams)
	require.NoError(t, err)
...
func makeP2WKH(t *testing.T, pubKey *crypto.PublicKey) []byte {
	pkHash := chainutil.Hash160(pubKey.SerializeCompressed())

	addr, err := chainutil.NewAddressWitnessPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2WSH(t *testing.T, pubKey *crypto.PublicKey) []byte {
	witnessScript := scriptP2PKH(t, pubKey)
	scriptHash := sha256.Sum256(witnessScript)

	addr, err := chainutil.NewAddressWitnessScriptHash(
...
func scriptP2PKH(t *testing.T, pubKey *crypto.PublicKey) []byte {
...
func doubleKey(key *crypto.PublicKey) *crypto.PublicKey {
	var keyJacobian crypto.JacobianPoint
	key.AsJacobian(&keyJacobian)

	crypto.DoubleNonConst(&keyJacobian, &keyJacobian)

	keyJacobian.ToAffine()

	return crypto.NewPublicKey(&keyJacobian.X, &keyJacobian.Y)
}
