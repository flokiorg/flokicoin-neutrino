...
func TestHeaderMetadataStorage(t *testing.T) {
	...
	testCases := []struct {
		name         string
		networkMagic wire.FlokicoinNet
		version      uint8
...
func TestLightHeaderCtxParent(t *testing.T) {
	...
	// Create a mock import source that returns headers by index.
...
func TestValidatePairWithNonMonotonicTimestamps(t *testing.T) {
	...
	// Create mock import source returning all headers by index.
	mockSource := &mockHeaderImportSource{}
	for i := 0; i < numHeaders; i++ {
		mockSource.On("GetHeader", uint32(i)).Return(
			headers[i], nil,
	...
	// Validate without BFFastAdd -- this exercises the median time check
	// in CheckBlockHeaderContext that previously failed. BFNoPoWCheck is
	// set since these synthetic headers aren't real mined headers and
	// this test isn't exercising the PoW-hash check.
	validator := newBlockHeadersImportSourceValidator(
		chaincfg.SimNetParams, mockStore,
		blockchain.BFNoPoWCheck, mockSource,
	)
