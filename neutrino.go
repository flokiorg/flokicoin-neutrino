...
var (
	...
	UserAgentVersion = "0.16.1-2-beta"
	...
	// DisableDNSSeed disables getting initial addresses for Flokicoin nodes
	// from DNS.
...
func isDevNetwork(net wire.FlokicoinNet) bool {
...
func (sp *ServerPeer) addKnownAddresses(addresses []*wire.NetAddressV2) {
	for _, na := range addresses {
		_, err := sp.knownAddresses.Put(
			netaddr.NetAddressKey(na), &cachedAddr{},
		)
		if err != nil {
...
// OnReject is invoked when a peer receives a reject flokicoin message and is
...
func (sp *ServerPeer) OnAddr(_ *peer.Peer, msg *wire.MsgAddr) {
	...
	for _, na := range msg.AddrList {
		...
		// is what is used by the netaddr.
...
// SubscribeRecvMsg adds a OnRead subscription to the peer. All flokicoin
...
// flokicoin network type specified by chainParams.  Use start to begin syncing
// with peers.
func NewChainService(cfg Config) (*ChainService, error) {
	...
	// network.
	amgr := netaddr.New(cfg.DataDir, nameResolver)
	...
	if !isDevNetwork(s.chainParams.Net) {
		newAddressFunc = func() (net.Addr, error) {
			...
			for _, peer := range s.Peers() {
				peerAddr := netaddr.NetAddressKey(peer.NA())
